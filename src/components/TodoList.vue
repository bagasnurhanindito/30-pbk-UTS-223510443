<template>
  <div class="body">
    <section class="create-todo">
      <h3>PRAPESAN MOTOR YAMAHA</h3>
      <form @submit.prevent="addTodo">
        <h4>KETIK MOTOR YANG INGIN ANDA PESAN</h4>
        <input type="text" placeholder="Masukkan kegiatan" v-model="inputContent" />
        <h4>Pilih kategori motor:</h4>
        <div class="options">
          <label>
            <input type="radio" name="category" value="MATIC" v-model="inputCategory" />
            <span class="bubble matic"></span>
            <div>MATIC</div>
          </label>
          <label>
            <input type="radio" name="category" value="SPORT" v-model="inputCategory" />
            <span class="bubble sport"></span>
            <div>SPORT</div>
          </label>
          <label>
            <input type="radio" name="category" value="BEBEK" v-model="inputCategory" />
            <span class="bubble bebek"></span>
            <div>BEBEK</div>
          </label>
        </div>
...
.body {
  ...
  align-items: center;
  min-height: 100vh;
  background-image: linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url('https://wallpapercave.com/wp/wp4585272.jpg');
